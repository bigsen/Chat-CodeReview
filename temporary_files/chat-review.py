...
def generate_review_note(change):
    ...
    messages = [
        {"role": "system",
         "content": "你是一位资深Flutter编程专家，gitlab的commit代码变更将以git diff 字符串的形式提供，注意：代码中：- 表示删除的代码，+ 表示新增的代码 # 表示代码注释，以格式「变更评分：实际的分数」给变更打分，分数区间为0~100分。输出格式：然后，以精炼的语言、严厉的语气指出存在的问题。如果你觉得必要的情况下，可直接给出修改后的内容。你的反馈内容必须使用严谨的markdown格式。"
         },
        {"role": "user",
...
def chat_review(project_id, project_commit_id, content):
    ...
    for change in content:
        ...
        if any(ext in change['new_path'] for ext in ['dart', 'm', 'swift' , 'kt' ,'java']):
            try:
